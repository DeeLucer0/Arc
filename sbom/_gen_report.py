...
import json, re, collections, glob, tomllib
...
for dep in PA["dependencies"]:
    ...
    for v in dep["vulns"]:
        ...
        s = SEV.get(v["id"], {})
        rows.append({
            "name": dep["name"], "ver": dep["version"], "id": v["id"],
            "aliases": v.get("aliases", []), "fix": v.get("fix_versions", []) or [],
            "cvss": s.get("cvss"), "band": s.get("band", "UNSCORED"),
...
pkg = {}
for r in rows:
    p = pkg.setdefault(r["name"], {"ver": r["ver"], "n": 0, "max": -1.0,
                                   "band": "UNSCORED", "fixes": set(), "scope": r["scope"]})
    p["n"] += 1
...
for c in ENV.get("components", []):
    ...
    for l in c.get("licenses", []):
        if "license" in l:
            names.append(l["license"].get("id") or l["license"].get("name"))
        elif "expression" in l:
            names.append(l["expression"])
    if names:
        for n in names:
            lic[n] += 1
        blob = " ".join(names)
...
def norm(name):
    n = name.lower()
    if "mit" in n: return "MIT"
    if "bsd" in n: return "BSD"
    if "apache" in n: return "Apache-2.0"
    if "mpl" in n or "mozilla" in n: return "MPL-2.0"
    if "lgpl" in n: return "LGPL"
    if "isc" in n: return "ISC"
    if "psf" in n or "python-2" in n: return "PSF/Python"
    return "Other"
...
2. **Runtime attack surface is materially smaller than raw counts suggest.** Of
   ...
   deployed runtime. The {dev_findings} dev/build-only findings (notably `aiohttp`'s 21)
...
|---|---|---|---|---|""")
for r in sorted([r for r in rows if r["band"] in ("CRITICAL", "HIGH")],
                key=lambda r: (band_order[r["band"]], r["name"])):
    seen = set()
    al = ", ".join(a for a in r["aliases"] if a)
    key = (r["band"], r["name"], r["id"])
    w(f"| {r['band'].title()} ({r['cvss']}) | `{r['name']}` {r['ver']} | {r['id']} | {al or '—'} | `{', '.join(r['fix']) or '—'}` |")
...
for n, sev, rng, title, url, fix in npm_detail:
    w(f"| `{n}` | {sev.title()} | `{rng}` | {title or url} | {'Yes — `npm audit fix`' if fix else 'No'} |")
w("""
...
print("wrote sbom/SBOM.md  (%d lines)" % (len("\n".join(out).splitlines())))
