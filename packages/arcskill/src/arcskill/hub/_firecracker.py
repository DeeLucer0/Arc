"""Firecracker microVM lifecycle for arcskill dry-run.

Sibling of ``arcskill.hub.dry_run``. The orchestrator there decides when
to use Firecracker; this module owns *how*: availability detection, the
config dataclass, the FirecrackerSandbox class that wraps the jailer
binary, and the ``_run_firecracker`` helper that bridges them with the
public ``run_dry_run`` flow.

Re-exported through ``arcskill.hub.dry_run`` for backward-compatible
imports — tests and callers continue to do
``from arcskill.hub.dry_run import FirecrackerSandbox`` etc.
"""
...
# ---------------------------------------------------------------------------
# _run_firecracker — bridge between run_dry_run and FirecrackerSandbox
# ---------------------------------------------------------------------------
